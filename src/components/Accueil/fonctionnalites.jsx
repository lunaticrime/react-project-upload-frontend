          ...
          {/* Features Section */}
            ...
            {features.map(({ id, title, description }) => (
              ...
              <LeftToRight delay={(6-id) * 0.4}>
                <Card id={id} title={title} description={description} isDarkMode={isDarkMode} />
              </LeftToRight>
            ))}
          </div>
...
const Card = ({ id, title, description, isDarkMode }) => {
  return (
    <motion.div
      ...
      className={`relative h-80 w-full max-w-[15rem] shrink-0 overflow-hidden rounded-2xl ${isDarkMode ? "bg-blue-1-dark" : "bg-blue-1"} p-8 cursor-default`}
    >
